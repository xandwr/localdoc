    ...
    match ensure_localdoc_initialized() {
        Ok(localdoc_dir) => {
            match cli.command {
                ...
                Some(Commands::Query { text, limit, json, entry_type, path, docpack, verbose }) => {
                    if !json {
                ...
                Some(Commands::Config { action }) => {
                    match action {
                        ConfigAction::Set { key, value } => {
                            if key == "telemetry" {
                                ...
                                match value.to_lowercase().as_str() {
                                    ...
                                    _ => {
                                        eprintln!("❌ Invalid value. Use: true/false, on/off, yes/no, or 1/0");
                                        std::process::exit(1);
                                    }
                                }
                            } else {
                        ...
                        ConfigAction::Show => {
                            ...
                            println!("{}", telemetry::telemetry_status());
                            println!("\n📁 Config location: {}", telemetry::get_config_path().display());
                        }
                    }
                }
                None => {
