...
pub fn query_docpacks(search_dirs: &[PathBuf], query: &str, options: &QueryOptions) -> Result<Vec<SearchResult>, std::io::Error> {
    ...
    for docpack_dir in &docpack_dirs {
        if let Ok(docpack_results) = search_docpack(docpack_dir, &query_lower, options, &mut all_terms) {
...
fn search_docpack(
    ...
    if let Some(filter_docpack) = options.docpack {
        if !docpack_name.to_lowercase().contains(&filter_docpack.to_lowercase()) {
            return Ok(results);
        }
    }
...
fn matches_filters(entry: &DocEntry, _docpack_name: &str, options: &QueryOptions) -> bool {
    ...
    if let Some(filter_path) = options.path {
        if !entry.path.to_lowercase().contains(&filter_path.to_lowercase()) {
            return false;
        }
    }
...
fn calculate_match_score(entry: &DocEntry, query: &str) -> Option<f32> {
    ...
    // Multi-word: check if name contains all words
        ...
        let matches = query_words.iter().filter(|&word| name_lower.contains(word)).count();
        if matches > 0 {
    ...
    if content_lower.contains(&query_lower) {
        ...
        let matches = query_words.iter().filter(|&word| content_lower.contains(word)).count();
        if matches > 0 {
